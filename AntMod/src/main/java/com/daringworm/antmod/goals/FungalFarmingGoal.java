...
public class FungalFarmingGoal<T extends LivingEntity> extends Goal {
    ...
    public void tick() {
        ...
        boolean isHandFungusFood = ant.getMainHandItem().getItem() == ModBlocks.MOLDY_LEAVES.get().asItem() || ant.getMainHandItem().getItem() == ModBlocks.WING_DEBRIS.get().asItem();
        ...
        if (aiTimer>10 && ant.getWorkingStage() == 0 && ant.level.getNearestPlayer(ant, 120) != null) {
            ...
            if(fungusPosList.isEmpty() || (advancedCheckTimer > 5 && dist2home<10)){
                fungusPosList = AntUtils.findAllBlockPos(ModBlocks.FUNGUS.get(),ant.blockPosition(),10,6,ant.level);
            }
            ...
            if(fungusAdjacentPosList.isEmpty() && !fungusPosList.isEmpty()){
                ...
                for(BlockPos tempPos : fungusAdjacentPosList){
                    ...
                    if(!AntUtils.isAdjacentTo(ant.level,tempPos,ModBlocks.FUNGUS.get(),true)){
                        cPosTemp.add(tempPos);
                    }
            ...
            if(!fungusAdjacentPosList.isEmpty() && ant.getHunger() > minAllowedHunger) {
                ...
                isHandFungusFood = ant.getMainHandItem().getItem() == ModBlocks.MOLDY_LEAVES.get().asItem() || ant.getMainHandItem().getItem() == ModBlocks.WING_DEBRIS.get().asItem();
                if (isHandFungusFood) {
                    ...
                    if (placePos != BlockPos.ZERO) {
                        ...
                        if (AntUtils.getDist(placePos, antPos) < distToInteract) {
                            if (ant.level.getBlockState(placePos).getRenderShape() == RenderShape.INVISIBLE) {
                                if(ant.getMainHandItem().getItem() == ModBlocks.MOLDY_LEAVES.get().asItem()) {
                                    ant.level.setBlock(placePos, ModBlocks.MOLDY_LEAVES.get().defaultBlockState(), 2);
                                }
            ...
            if(ant.getHunger()<minAllowedHunger && ant.getMainHandItem().isEmpty()){
                ...
                if(pStateUnder.getBlock() == ModBlocks.FUNGUS.get()){foodPos = ant.blockPosition();pState = pStateUnder;}

                if(pState.getBlock() == ModBlocks.FUNGUS.get()) {
